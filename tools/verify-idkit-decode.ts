...
function decodeIdkNew(idk: Buffer): { type: number; models: number[] } | null {
    let pos = 0; let type = -1; let models: number[] = [];
    try {
        while (pos < idk.length) {
            const code = idk[pos++]; if (code === 0) break;
            if (code === 1) { const n = idk[pos++]; for (let i = 0; i < n; i++) { models.push(idk.readUInt16BE(pos)); pos += 2; } }
            else if (code === 12) { type = idk[pos++]; }
            else if (code === 3) { /* disable */ }
            else if (code === 40) { const n = idk[pos++]; pos += n * 4; }
            else if (code === 60) { const n = idk[pos++]; pos += n * 2; }
...
sock.on('connect', async () => {
    ...
    for (let f = 0; f < Math.min(12, files.length); f++) {
        ...
        if (dec === null) { console.log(`  idk ${f}: DECODE FAILED (unknown opcode / overrun)`); continue; }
